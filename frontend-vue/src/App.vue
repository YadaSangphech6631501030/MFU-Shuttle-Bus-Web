 ...
 user
  ...
  if (selectedFrom && selectedTo && selectedPath && selectedPath.length > 1) {
    ...
    if (fromIndex < toIndex) {
      mapPolylines.push(new window.google.maps.Polyline({
        path: [{ lat: selectedFrom.lat, lng: selectedFrom.lng }, ...selectedPath.slice(fromIndex + 1, toIndex), { lat: selectedTo.lat, lng: selectedTo.lng }],
        geodesic: true,
        strokeColor: '#2196f3',
        strokeOpacity: 1,
        strokeWeight: 6,
...
watch([selectedLine, stations, buses, selectedFromId, selectedToId], renderGoogleMap, { deep: true });
